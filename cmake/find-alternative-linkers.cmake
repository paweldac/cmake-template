...
if (NOT PREFERED_LINKER)
    set(PREFERED_LINKER "None" CACHE STRING
        "Prefered linker during auto-detection, choose from ${LLD_LINKER_ID}, ${GNU_GOLD_LINKER_ID} and None" FORCE)
    set_property(CACHE PREFERED_LINKER PROPERTY STRINGS
        ${LLD_LINKER_ID} ${GNU_GOLD_LINKER_ID} "None")
endif()
...
execute_process(
    ...
    ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE OUTPUT_VARIABLE TRY_DETECT_LLD_LINKER
)

execute_process(
    COMMAND ${CMAKE_CXX_COMPILER} ${GOLD_LINKER_FLAG} -Wl,-v
    ERROR_QUIET OUTPUT_STRIP_TRAILING_WHITESPACE OUTPUT_VARIABLE TRY_DETECT_GNU_GOLD_LINKER
)

set(USER_PROVIDED_LINKER_FLAG "-fuse-ld=")
if ((${CMAKE_EXE_LINKER_FLAGS} MATCHES ${USER_PROVIDED_LINKER_FLAG}) OR
    (${CMAKE_SHARED_LINKER_FLAGS} MATCHES ${USER_PROVIDED_LINKER_FLAG}) OR
    (${CMAKE_CXX_FLAGS} MATCHES ${USER_PROVIDED_LINKER_FLAG}))
...
if (NOT USER_PROVIDED_LINKER AND AUTODETECT_ALTERNATIVE_LINKERS)
    ...
    if ((DETECTED_BOTH_LINKERS AND LLD_IS_PREFERED) OR
        (CXX_SET_TO_CLANG AND DETECTED_BOTH_LINKERS AND PREFERED_LINKED_NOT_SET) OR
        (DETECTED_LLD_LINKER AND NOT DETECTED_GNU_GOLD_LINKER))
        set(PROJECT_LINKER ${TRY_DETECT_LLD_LINKER})
        add_link_options(${LLD_LINKER_FLAG})
    endif()

    if ((DETECTED_BOTH_LINKERS AND GNU_GOLD_IS_PREFERED) OR
        (CXX_SET_TO_GNU AND DETECTED_BOTH_LINKERS AND PREFERED_LINKED_NOT_SET) OR
        (DETECTED_GNU_GOLD_LINKER AND NOT DETECTED_LLD_LINKER))
